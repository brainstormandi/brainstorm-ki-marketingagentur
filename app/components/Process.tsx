            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="grid lg:grid-cols-3 gap-16 relative">
                    ...
                    {PROCESS_STEPS.map((step, index) => (
                        <div
                            key={index}
                            className="relative z-10 flex flex-col items-center text-center group animate-reveal-up"
                            style={{ animationDelay: `${(index + 1) * 200}ms` }}
                        >
                            <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center font-black text-3xl mb-10 shadow-xl shadow-slate-200/50 group-hover:scale-110 group-hover:bg-accent group-hover:text-primary transition-all duration-500 border border-white">
                                {index + 1}
                            </div>
